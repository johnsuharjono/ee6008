...
import { registerProjects } from '@/src/app/actions/student/register'
import { ProjectCard } from '@/src/components/student/planner/project-card'
...
export function CardContainer({ plans, projectApplicationLimit }: CardContainerProps) {
    ...
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
      <SortableContext items={items} strategy={rectSortingStrategy}>
        <div className='grid md:grid-cols-3 gap-4'>
          {items.map((item, i) => (
            <ProjectCard key={item.id} data={item} index={i} isActive={i < projectApplicationLimit} />
          ))}
        </div>
      </SortableContext>
