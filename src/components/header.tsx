...
function Header({ title, description, className, actions }: HeaderProps) {
  return (
    <div className={cn('col-span-full mr-12 flex items-start justify-between lg:mr-0', className)}>
      <div className='flex w-full flex-col gap-2'>
        <h1 className='text-3xl font-semibold'>{title}</h1>
        {description ? <p className='text-muted-foreground'>{description}</p> : null}
      </div>
      {actions ? <div className='flex flex-1 items-center gap-2'>{actions}</div> : null}
    </div>
  )
}
